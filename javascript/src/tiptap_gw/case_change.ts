...
const CaseChange = Extension.create({
    ...
    addCommands() {
        return {
            changeCase: (newCase: CaseType) => ({state, tr}) => {
                    ...
                    state.doc.nodesBetween(
                        ...
                        (node, pos) => {
                            ...
                            if(range.$to.pos <= replaceTo) {
                                ...
                                oldText = oldText.slice(0, range.$to.pos - pos);
                                replaceTo = range.$to.pos;
                            }
                            if(replaceFrom < range.$from.pos) {
                                // Trim to selection start
                                oldText = oldText.slice(range.$from.pos - replaceFrom);
                                replaceFrom = range.$from.pos;
                            }
                            if(oldText)
                                inserts.push([oldText, replaceFrom, replaceTo]);
                            return false;
                        }
                    );
                }
                ...
                for(const [oldText, from, to] of inserts) {
                    ...
                    if(newCase === "lower") {
                        ...
                        throw new Error("Unrecognized case type: " + newCase);
                    }
                    tr.insertText(
                        newText,
                        tr.mapping.map(from),

...
<Info>
  ...
  experience, we recommend the higher system requirements for production use. That is equvilant to a `t3.xlarge` instance in AWS.
</Info>
...
<Steps>
  ...
  <Step title="Download Ghostwriter CLI">
    ...
    <CodeGroup>
      ...
      ```bash Mac
      curl -L -o ghostwriter-cli-darwin-arm64.tar.gz https://github.com/GhostManager/ghostwriter_cli/releases/latest/download/ghostwriter-cli-cli-darwin-arm64.tar.gz
      ```
    </CodeGroup>

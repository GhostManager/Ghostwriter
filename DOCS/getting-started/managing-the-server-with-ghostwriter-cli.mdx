...
Ghostwriter CLI reads and writes to the `.env` file and tries to establish an outbound network connection when you run
    the `update` command. The CLI only writes to Ghostwriter's `.env` file and only reads Ghostwriter's `.env` and
    `VERSION` files. For network connectivity, it only connects to GitHub's REST API to pull the version information
    for Ghostwriter's latest tagged release. However, these actions taken by a downloaded binary will cause some
    anti-malware and EDR solutions to flag Ghostwriter CLI as malware. If you encounter an issue with such a solution
    blocking Ghostwriter CLI from executing, please add the binary to your allowlist or exclusions list. We are working
    to try to get Ghostwriter CLI to be trusted, as it once was, but these issues come and go.

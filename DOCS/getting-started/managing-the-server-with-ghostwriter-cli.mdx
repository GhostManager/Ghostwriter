...
Ghostwriter CLI reads and writes to the `.env` file and tries to establish an outbound network connection when you run
    the `update` and `version` commands. The CLI only reads and writes to Ghostwriter's `.env` file. For network connectivity,
    it only connects to GitHub's REST API to pull the version information for Ghostwriter and Ghostwriter CLI's latest
    tagged releases. However, these actions taken by a downloaded binary will cause some anti-malware and EDR solutions to
    flag Ghostwriter CLI as malware. If you encounter an issue with such a solution blocking Ghostwriter CLI from executing,
    please add the binary to your allowlist or exclusions list.

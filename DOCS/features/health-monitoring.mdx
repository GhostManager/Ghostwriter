...
You can configure these using either:

1. `.env` or with Ghostwriter CLI (this sets and reads from `.env`)
2. Config file under `config/settings/`

For changes in `.env` or using the CLI, bring containers `down` and `up`. Changes made to config files will require a `build`. Use of config files will cause `.env` and CLI values to be ignored. So, if you use config files, you must use that for all health check values. In other words, if you want to use `.env`/CLI, then you cannot use config files.
...
Example config file `config/settings/production.d/10-healthchecks.py` snippet:

...
def GenerateMockProject(
    ...
    # Generate a representative client and project
    ...
    # Distribute findings across severities and assignees deterministically.
    severities = [
    ...
    # Generate deliberate scope and target permutations.
    for index in range(num_of_scopes):
        ProjectScopeFactory(
            project=project,
            disallowed=index % 3 == 1,
            requires_caution=index % 3 == 2,
        )

    for index in range(num_of_targets):
        ProjectTargetFactory(project=project, compromised=index % 2 == 0)

    # Distribute objectives across priorities, statuses, and completion states.
    obj_priorities = [
    ...
    objectives = [
        ProjectObjectiveFactory(
            project=project,
            priority=obj_priorities[index % len(obj_priorities)],
            status=obj_status[index % len(obj_status)],
            complete=index % 2 == 0,
        )
        for index in range(num_of_objectives)
    ]

    # Generate subtasks for each objective
    for objective_index, obj in enumerate(objectives):
        for subtask_index in range(num_of_subtasks):
            ProjectSubtaskFactory(
                parent=obj,
                status=obj_status[(objective_index + subtask_index) % len(obj_status)],
                complete=subtask_index % 2 == 0,
            )

    # Generate domain names and servers used for this project.
    ...
    for index in range(num_of_deconflictions):
        DeconflictionFactory(
            project=project,
            status=deconfliction_status[index % len(deconfliction_status)],
        )
    ...
    for index, domain in enumerate(domains):
        if index % 2 == 0:
            DomainServerConnectionFactory(
                domain=domain,
                static_server=servers[index % len(servers)],
                transient_server=None,
            )
        else:
            DomainServerConnectionFactory(

...
class ProjectRolePositionMigrationTests(TransactionTestCase):
    ...
    def test_migration_preserves_legacy_alphabetical_role_ordering(self):
        ProjectRole = self.apps.get_model("rolodex", "ProjectRole")

        self.assertEqual(
            list(ProjectRole.objects.order_by("position").values_list("project_role", flat=True)),
            ["First Custom Role", "Second Custom Role"],
        )


class ProjectRolePositionMigrationFallbackTests(TransactionTestCase):
    """Verify the migration creates a fallback role when assignments exist without any roles."""
    ...
    def setUpBeforeMigration(self, apps):
        ...
        self.client = Client.objects.create(name="Fallback Client")
        self.project_type = ProjectType.objects.create(project_type="Fallback Test")
        self.project = Project.objects.create(
            client=self.client,
            project_type=self.project_type,
            start_date=date.today(),
    ...
    def test_migration_creates_fallback_role_for_null_assignments(self):
        ProjectRole = self.apps.get_model("rolodex", "ProjectRole")
        ProjectAssignment = self.apps.get_model("rolodex", "ProjectAssignment")

        self.assertEqual(
            list(ProjectRole.objects.values_list("project_role", "position")),
            [("Unassigned", 1)],
        )

        migrated_assignment = ProjectAssignment.objects.get(pk=self.assignment.pk)
        self.assertEqual(migrated_assignment.role.project_role, "Unassigned")
